...
var registerDevice = function (client, message) {

    client.device_id = message.device_id;

    addClientToChannel(client.id, message.device_id);

    if (registeredDevices[message.device_id]) {

        if (settings.debug) {

            console.log('Reusing existing device registration for device:', message.device_id, ', client id:', client.id);
        ...
        message.messageType = 'device_register';
        message.clientId = client.id;
        message['uid'] = (this.uid) ? this.uid : 0;
        message['remote_address'] = client.conn.remoteAddress

        sendMessageToBackend(message, registerDeviceCallback);

    }

};

var genericMessageHandlerCallback = function (error, response, body) {
...
/**
 * Handle device registration reponse
 * */
var registerDeviceCallback = function (error, response, body) {
    return 'test';
    //console.log('registerDeviceCallback called with arguments',arguments);


    //return;

    //were translating a device registration to an object that is like a user. devices are users (for now)

    if (error) {

        console.log('Error with device registration', error);
        return;
    ...
    if (response.statusCode == 404) {
        ...
        if (settings.debug) {
            console.log('Backend registration url not found, full response info:', response);
        }
        else {
            console.log('Backend registration url not found.');
        }
        return;
    }


    var registrationData = false;
    try {

        registrationData = JSON.parse(body);
        response.client.emit('registered', registrationData);

    } catch (exception) {
        console.log('Failed to parse registration message:', exception);
...
var removeClientFromChannel = function (sessionId, channel) {
    if (sessionId && channel) {
        ...
        else if (!channels.hasOwnProperty(channel)) {
...
var setDeviceOffline = function (device_id, message) {
...
io.on('connection', function (socket) {
    ...
    socket.on('register_device', function (message) {
        ...
        if (sockets[socket.id]) {
            ...
            if (settings.debug) {
                ...
                console.log('Registering device by id ', message.device_id);
    ...
    socket.on('message', function (message) {
        ...
        if (sockets[socket.id] && (message.hasOwnProperty('type') || message.hasOwnProperty('messageType'))) {
            if (settings.debug) {
            ...
            // that this is allowed in the server configuration.
                ...
                message['uid'] = (socket.uid) ? socket.uid : (this.uid) ? this.uid : (message.uid) ? message.uid : 0;
    ...
    socket.on('disconnect', function () {
        ...
        if (socket.device_id) {
            ...
            var message = {
                ...
                'remote_address': this.conn.remoteAddress
            };

            setDeviceOffline(socket.device_id, message);
